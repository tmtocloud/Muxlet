...
local function _themeExportPath(name)
    if not getMudletHomeDir then return nil end
    local safe = name:gsub("[^%w_%-]", "_")
    return getMudletHomeDir() .. "/Muxlet_persistent/themes/" .. safe .. ".lua", safe
end
...
-- Serialize a token table to a standalone registerTheme() Lua script.
local function _serializeTheme(name, tok)
    local keys = {}; for k in pairs(tok) do keys[#keys+1] = k end; table.sort(keys)
    local lines = {
        "-- Muxlet theme \"" .. name .. "\" — generated by `mux theme save`.",
        "-- Add this as a script in your package to ship the theme.",
        "if not Mux or not Mux.registerTheme then return end",
        string.format("Mux.registerTheme(%q, {", name),
    }
    for _, k in ipairs(keys) do
        local v, vs = tok[k], nil
        if     type(v) == "string"  then vs = string.format("%q", v)
        elseif type(v) == "number"  then vs = tostring(v)
        elseif type(v) == "boolean" then vs = tostring(v) end
        if vs then lines[#lines+1] = string.format("    [%q] = %s,", k, vs) end
    end
    lines[#lines+1] = "})"
...
function Mux.saveThemeFromGlobals(name)
    ...
    local exportPath, safe = _themeExportPath(name)
    if exportPath and io then
        pcall(function() if lfs then lfs.mkdir(getMudletHomeDir() .. "/Muxlet_persistent/themes") end end)
        pcall(function()
            local f = io.open(exportPath, "w")
            if f then f:write(_serializeTheme(name, snap)); f:close() end
        end)
    end
