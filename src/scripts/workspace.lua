...
local function serializeLua(value, indent)
    ...
    if valueType ~= "table" then
        error("Mux.exportWorkspace: cannot serialize value of type " .. valueType)
    end
    ...
    if isArrayTable(value) then
        for _, v in ipairs(value) do
            lines[#lines + 1] = padIn .. serializeLua(v, indent + 1)
        end
    else
        local keys = {}
        ...
        for _, k in ipairs(keys) do
            lines[#lines + 1] = padIn .. luaKey(k) .. " = " .. serializeLua(value[k], indent + 1)
        end
    end
...
function Mux.exportWorkspace(name)
    ...
    local lua = table.concat({
        ...
        "-- changing the layout in-game and re-saving with `mux workspace save " .. name .. "`.",
        "",
        "Mux.registerWorkspace(" .. string.format("%q", name) .. ", " .. serializeLua(def, 0) .. ")",
        "",
    }, "\n")

    local outPath = Mux._persistentDir .. "/" .. name .. "-workspace-export.lua"
